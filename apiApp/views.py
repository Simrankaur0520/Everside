...
#--------------------------- Filters---------------------------------------------------
'pbkdf2_sha256$320000$ReiwVBU55wBIRWiSRf2hsH$c039yFSOS7G3AOvLt1AM96rX0K5wFd2mSC38faYEGHk='

@api_view(['GET'])
def demo_login(request):
    return Response(make_password('admin'))
    ...
    try:
        if request.method == 'POST':
            ...
            if username=="demo" and password=="1234":
                return Response({'Message':'TRUE','token':'nfsnoioismlkvnkj266567'})
            else:
                return Response({'message':'False'})
...
def providerCommentDownload(request,format=None):
    ...
    obj = everside_nps.objects.filter(TIMESTAMP__gte=startDate)\
                              .filter(TIMESTAMP__lte=sendDate)\
